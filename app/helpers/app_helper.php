    ...
    function get_option_lang($key = NULL) {
        ...
        if(isset($val->option_value)) {
           ...
           if($line != FALSE and isset($line[$lang])) {
              $line_str = str_replace(["{{baseurl}}","{{year}}"], [base_url(),date('Y')], $line[$lang]);
              return $line_str;
           } else {
              $line_str = str_replace(["{{baseurl}}","{{year}}"], [base_url(),date('Y')], $val->option_value);
              return $line_str;
           }
        }
        return "";
...
if( ! function_exists(('get_lang'))) {
    function get_lang($type = NULL,$item = NULL,$ref = NULL,$lang = 'id',$default = "") {
        ...
        return (isset($val->text)) ? $val->text : $default;
    }
}
...
if( ! function_exists('get_available_langs') ) {
    function get_available_langs() {
        $CI =& get_instance();
        $langs = $CI->db->order_by('keylang','desc')->get_where('language_options',['is_active'=>1])->result_array();

        if($langs == FALSE or count($langs) == 0) {
            return [];
        }
        ...
        <div class="form-group">
            <fieldset>
                ...
                <ul class="nav nav-tabs" role="tablist">
                    ...
                    $i = 0; 
                    foreach($langs as $key => $value): 
                        $id = (empty($custom_id)) ? str_replace(['[',']'],'',$name).$key : $custom_id.$key;
                    ?>
                    <li role="presentation" <?php if($i == 0) echo 'class="active"'; ?>><a href="#<?php echo $id; ?>" aria-controls="<?php echo $name.$key; ?>" role="tab" data-toggle="tab"><?php echo $value['label']; ?></a></li>
                    <?php $i++; endforeach; ?>
                </ul>
                ...
                <div class="tab-content">
                    ...
                    ?>
                    <div role="tabpanel" class="tab-pane <?php if($max_character > 0) echo "max-limit" ?> <?php if($i == 0) echo 'active'; ?>" id="<?php echo $id; ?>">
        ...
        <div class="form-group">
            <fieldset>
                ...
                <ul class="nav nav-tabs" role="tablist">
                    ...
                    <li role="presentation" <?php if($i == 0) echo 'class="active"'; ?>><a href="#<?php echo str_replace(['[',']'],'',$name).$key; ?>" aria-controls="<?php echo $name.$key; ?>" role="tab" data-toggle="tab"><?php echo $value['label']; ?></a></li>
                    <?php $i++; endforeach; ?>
                </ul>
                ...
                <div class="tab-content">
                    ...
                    ?>
                    <div role="tabpanel" class="tab-pane <?php if($i == 0) echo 'active'; ?>" id="<?php echo str_replace(['[',']'],'',$name).$key; ?>">
