...
class Project_model extends CI_Model
{
	...
	public function get_newest_project($max = 5) {
		$rows = $this->db->query("SELECT * FROM {$this->db->dbprefix('portofolio')} WHERE is_active = 1 AND is_pinned = 1 ORDER BY created_date DESC LIMIT 0,".$max)->result_array();
		
		foreach ($rows as $key => $value) {
			$rows[$key]['project_name'] = get_lang('portfolio','title',$value['id'],$this->lang,$value['project_name']);
			$rows[$key]['overview'] = get_lang('portfolio','overview',$value['id'],$this->lang,$value['overview']);

			$rows[$key]['thumb'] = site_url('medias/projects/'.$value['thumbnail']);
			$rows[$key]['url'] = site_url($this->lang.'/portfolio/'.$value['slug']);
			$rows[$key]['snippet'] = substr(strip_tags($rows[$key]['overview']), 0, 100);
		}
	...
	public function get_detail($slug = '') {
		...
		$get->project_name = get_lang('portfolio','title',$get->id,$this->lang,$get->project_name);
		$get->overview = get_lang('portfolio','overview',$get->id,$this->lang,$get->overview);
	...
	public function get_related_projects($category_ids = [], $max = 4) {
		...
		foreach ($get as $key => $value) {
			$get[$key]['project_name'] = get_lang('portfolio','title',$value['id'],$this->lang,$value['project_name']);
			$get[$key]['overview'] = get_lang('portfolio','overview',$value['id'],$this->lang,$value['overview']);

			$get[$key]['thumb'] = site_url('medias/projects/'.$value['thumbnail']);
			$get[$key]['url'] = site_url($this->lang.'/portfolio/'.$value['slug']);
