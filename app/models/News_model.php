...
class News_model extends CI_Model {
	...
	private function prepare($rows = [],$options = []) {
		...
		foreach ($rows as $key => $value) {
			...
			$rows[$key]['title_text'] = (!isset($value['title_text']) or empty($value['title_text'])) ? $value['title'] : $value['title_text'];
			$rows[$key]['content_text'] = (!isset($value['content_text']) or empty($value['content_text'])) ? $value['content'] : $value['content_text'];

			$rows[$key]['date_formatted'] = date('F jS Y',strtotime($value['publish_date']));
			$rows[$key]['url'] = site_url($this->lang.'/blog/'.$value['slug']);
