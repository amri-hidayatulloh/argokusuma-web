...
class Blog extends APP_Frontend {
	...
	public function index($lang = 'id', $category = 'all') {
		$this->set_lang($lang,'blog/'.$category);
		$query = $this->input->get('query_search');
		
		$this->_data['active_category'] = $category;
		$this->_data['blogs'] = $this->get_data(1,$category,$lang,$query,FALSE);
	...
	public function get_data($page = 1,$category_slugs = 'all',$lang = 'id',$query = '',$json = TRUE) {
		$offset = 0;
		$limit = 3;
		...
		$next_url = ($next_page > 0) ? site_url('blog/get_data/'.$next_page.'/'.$category_slugs.'/'.$lang.'/'.$query) : '';
