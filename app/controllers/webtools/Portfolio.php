...
class Portfolio extends APP_Webtools {
	...
	public function save() {
		...
		$overview = $this->get_input_lang('overview');
		...
		if(!empty($title) and !empty($client) and !empty($overview)) {
			...
			$datas = array(
						'project_name' => $title['maintext'],
						'slug' => slugify($title['maintext']),
						'client_id' => $client,
						'overview' => $overview['maintext'],
						'is_active' => $state,
						'is_pinned' => $pin
					 );
			...
			if(isset($save) and $save != FALSE) {
				$this->save_text('portfolio','title',$id,$title['lists']);
				$this->save_text('portfolio','overview',$id,$overview['lists']);
