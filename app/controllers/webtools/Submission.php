...
class Submission extends APP_Webtools {
	...
	public function suggestion($page = 1) {
		$keyword = $this->input->post('keyword');
		...
		$query = "SELECT * FROM {$this->db->dbprefix('suggest_submission')} WHERE is_active != 0 ORDER BY created_date DESC";

		if(!empty($keyword)) {
			$query = "SELECT * FROM {$this->db->dbprefix('suggest_submission')} WHERE is_active != 0 AND (name LIKE '%".$keyword."%' OR email LIKE '%".$keyword."%' OR suggestion LIKE '%".$keyword."%' OR phone LIKE '%".$keyword."%' ) ORDER BY created_date DESC";
		}
		...
		$this->_template_master_data['page_title'] = 'Suggest Submission';
		$this->_template_master_data['page_subtitle'] = '';
		$this->_addContent($this->_data);
		$this->_render();		
	}

	public function exportsuggestion() {
		set_time_limit(0);
		
		$lists = $this->db->query("SELECT * FROM {$this->db->dbprefix('suggest_submission')} WHERE is_active != 0  ORDER BY created_date ASC")->result_array();
	
		header('Content-Type: text/csv; charset=utf-8');
		header('Content-Disposition: attachment; filename=suggestion-'.date('Ymd-His').'.csv');
	...
	public function deletesuggestion($id = 0) {
		$return = ['code'=>200];
		$set = $this->db->update('suggest_submission',['is_active'=>0],['id'=>$id]);
