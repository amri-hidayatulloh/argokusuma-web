...
class Teams extends APP_Webtools {
	...
	public function index() {
		...
		$this->_data['lists'] = $this->db->query("SELECT s.* FROM {$this->db->dbprefix('teams')} s WHERE s.is_active = 1 AND s.group_type = 'team'")->result_array();
	...
	public function action($action = 'add',$id = NULL) {
		...
		if($id != NULL) {
			$check = $this->db->query("SELECT s.* FROM {$this->db->dbprefix('teams')} s WHERE s.is_active != 0 AND s.id = '".$id."'  AND s.group_type = 'team'")->row();
			if(isset($check->id)) {
