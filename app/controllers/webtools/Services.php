...
class Services extends APP_Webtools {
	...
	public function save() {
		...
		$caption = $this->get_input_lang('caption');
		$title = $this->get_input_lang('title');
		$description = $this->get_input_lang('description');

		if(!empty($title['maintext']) and !empty($caption['maintext']) and !empty($description['maintext'])) {
			$datas = array(
						'title' => $title['maintext'],
						'slug' => slugify($title['maintext']),
						'caption' => $caption['maintext'],
			...
			if(isset($save) and $save != FALSE) {
				$this->save_text('services','title', $id, $title['lists']);
				$this->save_text('services','caption', $id, $caption['lists']);
				$this->save_text('services','description', $id, $description['lists']);
				...
				foreach ($countersegment as $key => $value) {
					...
					if(empty($idsegment)) {
						...
						$idsegment = $this->db->insert_id();
					} else {
						$save = $this->db->update('services_extends',$datas,['id'=>$idsegment]);
					}

					if(isset($save)) {
						$this->save_text('services-segment','title', $idsegment, $title);
						$this->save_text('services-segment','description', $idsegment, $desc);
					}
				}
