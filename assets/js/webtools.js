...
$(document).ready(function(){
	...
	function ajaxFormListener(autorefresh = true) {
		$('.ajax-form').off();
	...
	function removeThsiRowListener() {
		$(".remove-this-row").off('click');
		$(".remove-this-row").click(function() {
			$(this).parent().parent().remove();
		});
	}

	function addRowQuoteListener() {
		$(".addrowquote").off('click');
		$(".addrowquote").click(function(){
			var container = $(this).parent().parent().parent().parent().find('tbody.content');
			var template = $("#template-row-quote").html();

			container.append(template);
			removeThsiRowListener();

			return false;
		});
	}

	function removeQuoteListener() {
		$(".remove-quote").off('click');
		$(".remove-quote").click(function(){
			var btn = $(this),
				id = btn.data('id'),
				panel = btn.parent() //Div.col-md-12
							.parent() //Div.row
			...
			function(isConfirm){
				if (isConfirm) {
					if(id != "") {
						...
						$.get(baseurl+'pricing/deletequote/'+id, function(d) {
							d = JSON.parse(d);
	...
	$("#addnewquote").click(function(){
		var quote = $("#template-panel-quote").html();
		var num = $("#quote-area").find('.panel').size();

		num += 1;
		quote = quote.replace(/NUMERICALORDER/g,num);
	...
	$(".trigger-comment-delete").click(function(){
		...
		var _url = baseurl+_target+'/'+_method+'/'+_id;
